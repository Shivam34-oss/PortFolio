...
const MY_EMAIL = 'sc7601862@gmail.com'; // Your actual email
const MY_TWITTER_HANDLE = 'your-username'; // Your Twitter username
...
const SOCIAL = [
  ...
  { id: 'twitter', label: 'Twitter', href: `https://twitter.com/${MY_TWITTER_HANDLE}`, svg: (
    <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true" focusable="false">
  ...
  { id: 'email', label: 'Email', href: MY_EMAIL, svg: (
    <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true" focusable="false">
          ...
          <div className={styles.social} role="navigation" aria-label="Social links">
            {SOCIAL.map(s => (
              <button
                ...
                onClick={() => handleClick(s.id, s.href)}
                aria-label={s.label}
                title={s.id === 'email' ? `Copy email (${s.href})` : s.label}
              >
                <span className={styles.icon} aria-hidden="true">{s.svg}</span>
              </button>
            ))}
          </div>
          <div className={`${styles.copied} ${copied ? styles.show : ''}`} aria-hidden={!copied} role="alert">
            Email copied!
          </div>
        </div>

        <div className={styles.copy}>
          © {new Date().getFullYear()} Shivam Chauhan — <span className={styles.highlight}>Full-Stack Developer</span>
        </div>
      </div>
    </footer>
  )
